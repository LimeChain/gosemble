...
build-docker:
	set -e; \
	docker build --tag $(IMAGE):$(TAG)-extalloc-leak -f tinygo/Dockerfile.polkawasm tinygo; \
	docker run --rm -v $(CURRENT_DIR):$(SRC_DIR) -w $(SRC_DIR) $(IMAGE):$(TAG)-extalloc-leak /bin/bash -c "tinygo build -target=polkawasm -o=$(SRC_DIR)/$(BUILD_PATH) $(SRC_DIR)/runtime/"; \
	echo "Build - tinygo version: ${TAG}, gc: extalloc-leak"
