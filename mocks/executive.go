...
func (m *Executive) ApplyExtrinsic(uxt types.UncheckedExtrinsic) (primitives.DispatchOutcome, error) {
	args := m.Called(uxt)

	if args.Get(1) != nil {
		return args.Get(0).(primitives.DispatchOutcome), args.Get(1).(error)
	}
