...
func Test_CheckGenesis_AdditionalSigned_UnexpectedError(t *testing.T) {
	hash := primitives.Blake2bHash{
		FixedSequence: sc.BytesToFixedSequenceU8(
			common.MustHexToHash("0x88dc3417d5058ec4b4503e0c12ea1a0a89be200fe98922423d4334014fa6b0ff").ToBytes(),
		)}
	...
	mockModule.On("StorageBlockHash", sc.U64(0)).Return(hash, unexpectedError)

	result, err := target.AdditionalSigned()

	assert.Equal(t, primitives.NewTransactionValidityError(primitives.NewUnexpectedError(unexpectedError)), err)
	assert.Nil(t, result)
	mockModule.AssertCalled(t, "StorageBlockHash", sc.U64(0))
}
