...
func Test_ValidateTransaction_StaleError(t *testing.T) {
	...
	extra := newTestExtra(types.NewEra(types.ImmortalEra{}), 2, 0)
	call := newTestCall(0, 0, 0xab, 0xcd)
	signer := newTestSigner()
...
func Test_ValidateTransaction_Era(t *testing.T) {
	...
	setBlockNumber(t, storage, 17)
	// <BlockHash<Test>>::insert(16, H256::repeat_byte(1));

	extra := newTestExtra(types.NewMortalEra(16, 256), 0, 0)
	call := newTestCall(0, 0, 0xab, 0xcd)
	signer := newTestSigner()
	signature := newTestSignature(0xa3, 0xdb, 0xd9, 0x20, 0x08, 0x28, 0x0a, 0x74, 0x0b, 0x92, 0x63, 0x57, 0x93, 0x35, 0xbf, 0xcb, 0x53, 0x56, 0x35, 0x44, 0xde, 0x1e, 0x07, 0xbd, 0x9b, 0x60, 0x30, 0x7f, 0xd3, 0xfe, 0x24, 0x34, 0x05, 0xce, 0xb1, 0x5b, 0x5d, 0xcc, 0xe9, 0x61, 0xf0, 0x94, 0x8f, 0xd0, 0xf9, 0xac, 0xea, 0xf5, 0x62, 0xd5, 0xf8, 0x25, 0x83, 0xee, 0x55, 0x0e, 0xff, 0x91, 0x1d, 0x54, 0xc8, 0x42, 0xa6, 0x05)
	tx := types.NewSignedUncheckedExtrinsic(call, signer, signature, extra)
	txSource := types.NewTransactionSource(types.External)
	blockHash := types.Blake2bHash{FixedSequence: sc.BytesToFixedSequenceU8(parentHash.ToBytes())}
