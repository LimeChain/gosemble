...
// SignedExtension - Means by which a transaction may be extended. This type embodies both the data and the logic
...
type SignedExtension interface {
	...
	PreDispatch(who AccountId, call Call, info *DispatchInfo, length sc.Compact[sc.Numeric]) (Pre, error)
	...
	PostDispatch(pre sc.Option[Pre], info *DispatchInfo, postInfo *PostDispatchInfo, length sc.Compact[sc.Numeric], result *DispatchResult) error
